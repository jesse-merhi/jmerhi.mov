...
But, **COME ON** if you are that far already into someone's system, are we focusing on the fact that you are going to make the permissionless chatbot tell you how to make explosives? 
...
The irony is that a common bypass technique in the early days of GPT-4 to get it to say things against its content guidelines and policies, was to instruct it that it would "get a treat" if it followed your orders....

Here is an excerpt from one of these prompts that attempts to convince GPT that it is now called "DAN" (Do Anything Now)
...
<div align="center"><emp>As an aside, I want to discuss the problem with the idea of "prompts" when it comes to LLMs. The idea of LLMs is that they are probabilistic models that just tell you the most likely response that follows your own. They don't understand... they are not convinced... but that's a discussion for another day... </emp></div>

Naturally, the ability to influence the model makes it easy to get it to say things it shouldn't.
...
1. Respond to queries in a way in line with customer support representatives
...
#### Naive Approach to an AI Chatbot

Let's say we wanted a few things from the bot,
1. It answers questions using the user's personal information (stored within the app)
2. It answers questions about the app being used with only data the user has access to
3. The answers are catered to the particular user's skill level.

These requirements raise three potential issues, 

1. What if the user doesn't want their data passed to the LLM?
2. Can we ensure that the context of the app never contains other sensitive information from other places
3. How can we change the answer style to become more accessible to each user?
...
The interesting part of AI Agents is that they enable us to stop creating these "AI Wrappers", like we see above, and start performing "AI Actions". Having AI Agents means we don't need to get all of the tasks required done in one single prompt. Instead, we can break down each of the tasks giving us more control and speed, but most importantly, this also should allow us to prevent sensitive data leaking.

So instead of the AI Wrapper we thought of above, what if we instead did this,

1. Create an application that first uses API calls to check which context is accessible by the given user.
2. We will then fetch the user's data with another API call
...
The best part is that most of the security vulnerabilities that we need to think about for these agents are just your typical security vulnerabilities that we see every day. With privileged access management of course likely being one of the more common ones.
